                    ...
                    <div class="header-top-m">
                        <div class="left">
                            ...
                            <a href="{{ url('/') }}" style="color: white">
                                <img src="{{asset('public/styles/assets/images/uz.png')}}" width="24px" height="auto"
                                     alt="uz">
                            </a>
                        </div>
                        <div class="right">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="main-haeder-wrapper-h2">
                        <a href="{{ route('ru.index') }}" class="logo-area">
                        ...
                        <div class="header-nav main-nav-one">
                            <nav>
                                <ul>
                                    @php
                                        $menus = \Illuminate\Support\Facades\DB::select("Select `id`,`title`,`link` from `menus__ru` where parent_id = 0 ");
                                    @endphp
                                    @foreach($menus as $menu)

                                        @php
                                            $has_child = \Illuminate\Support\Facades\DB::select("Select * from `menus__ru` where parent_id = '$menu->id' ");
                                            ...
                                            @if(count($has_child)>0)
                                                <ul class="submenu">
                                                    @foreach($has_child as $child)
                                                        @php
                                                            $slugs = \Illuminate\Support\Facades\DB::select("Select * from `pages__ru` where `parent` = $child->id ");
                                                        @endphp
                                                        @if($slugs)
                                                            @php
                                                                $link = $slugs[0]->slug;
                                                            ...
                                                            <li>
                                                                <a style="padding: 10px 10px; font-weight: normal;"
                                                                   href="{{ route('ru.content', ['slug'=>$link]) }}">{{ $child->title }}</a>
                                                            </li>
                                                        @else
                                                            <li>
                                                                <a style="padding: 10px 10px; font-weight: normal;"
                        ...
                        <div class="actions-area">
                            ...
                            <a href="{{ route('ru.murojaat') }}" class="rts-btn btn-primary has-dropdown">Отправить
                                заявку</a>
                        </div>
                    </div>
                </div>
